...
@interface GDialog : UIView <UIWebViewDelegate> {
    id<GDialogDelegate> _delegate;
 ...
 * The delegate.
 */
@property (nonatomic, assign) id<GDialogDelegate> delegate;
...
- (id)initWithURL: (NSString *) loadingURL
  ...
  isViewInvisible: (BOOL) isViewInvisible
         delegate: (id <GDialogDelegate>) delegate;
...
/**
 * Subclasses should override to process data returned from the server in a 'ftconnect' url.
 *
 ...
 */
@protocol GDialogDelegate <NSObject>
